...
define('admin/plugins/session-sharing', ['settings'], function (Settings) {
	...
	ACP.showUserId = function () {
		...
		ACP._searchDelay = setTimeout(function () {
			...
			var qs = decodeURIComponent($.param({ query: element.val() }));
					...
					if (results.users.length) {
						socket.emit(
							...
							function (err, remoteIds) {
								if (err) {
									resultEl.text(
										'We encountered an error while servicing this request:' +
											err.message,
									);
								} else {
									resultEl.empty();
		...
		ACP._searchDelay = setTimeout(function () {
			...
			socket.emit(
				...
				function (err, results) {
					if (!err && results) {
						$('#local_result').html(
							'<div class="media"><div class="media-left"><a target="_blank" href="' +
								...
								(results.picture
									...
									: '<div class="avatar avatar-sm" style="background-color: ' +
										results['icon:bgColor'] +
										';">' +
										results['icon:text'] +
										'</div>') +
								'</a></div>' +
								'<div class="media-body"><a target="_blank" href="' +
								config.relative_path +
								'/user/' +
								results.userslug +
								'">' +
						...
						$('#local_result').text(
							'No users were found associated with that remote ID',
						);
					}
				},
			);
